...
const Card = ({invoiceId,name,dueDate, amount, status,darkMode}) => {
    ...
    return (
            <div className={`${darkMode ? 'bg-dark-light shadow' : ''} flex items-center px-10 
                ...
                <div className="md:flex md:justify-between md:items-center grid grid-cols-3  gap-2 w-full">
                    ...
                    <p className=" text-light-violet row-start-1 col-span-2 col-start-3
                        row-end-2 col-end-3">
                            {name}
                    </p>
                    <p className="pt-[.8rem] font-bold text-xl xl:text-xl row-start-3 col-start-1 row-end-4 col-end-3 row-span-2">£ {amount}</p>
                    <button className={"capitalize xl:w-36 xl:h-14 w-24 h-10 rounded-lg bg-opacity-5 dark:bg-opacity-20  row-start-3 col-start-3 row-end-4 col-end-3 "
